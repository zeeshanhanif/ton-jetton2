...
const getLastBlock = async (provider: NetworkProvider) => {
    return 1;//(await (provider.api() as TonClient4).getLastBlock()).last.seqno;
}
...
const changeAdminAction = async(provider:NetworkProvider, ui:UIProvider) => {
    ...
    const curState = await (provider.api() as TonClient4).getAccount(await getLastBlock(provider),minterContract.address);
...
export async function run(provider: NetworkProvider) {
    ...
    do {
        ...
        if(contractState.account.state.type !== "active" || contractState.account.state.code == null) {
            ...
            const stateCode = Cell.fromBoc(Buffer.from(contractState.account.state.code))[0];
